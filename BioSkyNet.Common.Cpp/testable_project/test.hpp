...
namespace services_api
{
	...
	public:
		...
		virtual void bind(grpc::ServerBuilder* server_builder)
		{
			logger_.info("Try open port on {0}", address_);

			server_builder->AddListeningPort(address_, grpc::InsecureServerCredentials());
	...
	protected:
		template<typename T>
		void add_rpc_handler(grpc::ServerBuilder* server_builder)
		{
			auto cq_ = server_builder->AddCompletionQueue();
